% some parameters that need to be check manually on imageJ
...
saveExcel=0;
%do some stuff to check if it works properly
test=0;
% select the images
[nomeFile,nomeCartella] = uigetfile('*.*','Select the image','MultiSelect','on');
if ~iscell(nomeFile)
nomeFile={nomeFile};
end
...
ifile=1;
%load the recording
nomeCompleto=[nomeCartella,nomeFile{ifile}];
